...
tab1_qtde_produto = tab1_qtde_produto.groupby('Produto vendido')['Quantidade'].sum().reset_index()
...
tab3_Vendas_Vendedor = tab3_Vendas_Vendedor.groupby('Vendedor')['Quantidade'].sum().reset_index()
#tab3_Vendas_Vendedor = tab3_Vendas_Vendedor.drop(columns=['Numero Pedido', 'Preço'])
...
tab4_Venda_Cliente = tab4_Venda_Cliente.groupby('Cliente')['Quantidade'].sum().reset_index()
